...
_SKIP_REMOVED = (
    "Skipped: relies on fields removed in Odoo 19 (rule_type, "
    "matching_order, match_nature, payment_tolerance, match_text_location_*, etc.)"
)


@unittest.skip(_SKIP_REMOVED)
...
class TestReconciliationMatchingRules(AccountTestInvoicingCommon):
    ...
    def setUpClass(cls):
        ...
        cls.invoice_line_3 = cls._create_invoice_line(
            300, cls.partner_1, "in_refund", name="RBILL/2019/09/0013"
        )
        ...
        cls.invoice_line_6 = cls._create_invoice_line(
            600, cls.partner_3, "out_invoice", ref="RF12 3456"
        )
        cls.invoice_line_7 = cls._create_invoice_line(
            200, cls.partner_3, "out_invoice", pay_reference="RF12 3456"
        )
    ...
    # Fields removed in Odoo 19: rule_type, auto_reconcile, match_nature,
    # match_same_currency, allow_payment_tolerance, payment_tolerance_type,
    # payment_tolerance_param, match_partner, match_text_location_*,
    ...
    def _create_reconcile_model(cls, **kwargs):
        # Strip removed v18 fields
        cleaned = {k: v for k, v in kwargs.items() if k not in cls._REMOVED_FIELDS}
        return cls.env["account.reconcile.model"].create(
            {
                ...
                **cleaned,
                "line_ids": [
                    ...
                    for i, line_vals in enumerate(cleaned.get("line_ids", []))
                ],
            }
        )
    ...
    def test_zero_payment_tolerance_auto_reconcile(self):
        ...
        for inv_type, bsl_sign in (("out_invoice", 1), ("in_invoice", -1)):
            ...
            # The payment amount is higher than the invoice one.
            st_line = self._create_st_line(
                amount=bsl_sign * 1010.0, payment_ref="123456"
            )
            self._check_statement_matching(
                rule,
    ...
    def test_allow_payment_tolerance_lower_amount(self):
        rule = self._create_reconcile_model(
            line_ids=[{"amount_type": "percentage_st_line"}]
        )
    ...
    def test_match_different_currencies(self):
        ...
        invoice_line = self._create_invoice_line(
            100, partner, "out_invoice", currency=currency_inv
        )
    ...
    def test_matching_with_write_off_foreign_currency(self):
        ...
        st_line = self._create_st_line(
            amount=100.0, payment_ref="123456", journal_id=journal_foreign_curr.id
        )
        self._check_statement_matching(
            reco_model,
            {
